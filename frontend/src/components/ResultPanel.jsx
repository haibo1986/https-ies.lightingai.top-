...
export default function ResultPanel({ result }) {
  ...
  return <section id="generation-result" className="card result-center appear" aria-labelledby="result-title">
    ...
    {result.centering_applied && result.centering && <p className="centering-note">已对中校正：各平面配光曲线已按自身峰值平移对中（最大偏移 {number(result.centering.max_shift_degrees, 1)}°），曲线形状与光束角保持不变。{result.centering.out_of_range_ratio > 0.1 && <b>注意：原垂直角范围有限，大偏移校正后部分方向数据缺失，结果可信度下降，建议核实。</b>}</p>}
